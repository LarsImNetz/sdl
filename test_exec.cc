/*-*-c++-*-
 *
 *    $RCSfile: test_exec.cc,v $
 *     Project:
 * Description:
 *
 *   Author(s): Lars Langhans
 *       $Date: 2008/10/24 17:57:00 $
 *   Copyright: (c)1996 by Lars Langhans
 *   $Revision: 1.2 $
 */

// damit kann RCS im executeable noch die Version feststellen
// static char rcsid[] = "$Id: test_exec.cc,v 1.2 2008/10/24 17:57:00 langha_l Exp $";
 ...
 *               -------
 *       $Log: test_exec.cc,v $
 *       Revision 1.2  2008/10/24 17:57:00  langha_l
 *       forktest
 *
...
void init()
{
    ...
    if (m_nPid == 0)
    {
        ...
        int n = execlp( "/usr/bin/mp3info", "mp3info", "-p", "%a\n%l\n%y\n%t", m_sFilename.c_str(), (const char*)NULL);
        std::cerr << "failure to call mp3info. It returns: " << errno << std::endl;
        exit(n);
...
int main(int argc, char* argv[], char* env[])
{
    ...
    std::string sResult;
    DoExec *pExec = new DoExec(argv[2]);
