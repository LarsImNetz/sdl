...
/* $Id: playmus.c,v 1.1 2006/04/26 15:25:34 langha_l Exp $ */
...
{
	t_print(T_DEBUG, "Usage: %s [-i] [-l] [-8] [-r rate] [-b buffers] [-v N] <musicfile>\n", argv0);
}
...
int main(int argc, char *argv[])
{
	...
	if ( SDL_Init(SDL_INIT_AUDIO) < 0 ) {
		t_print(T_DEBUG, "Couldn't initialize SDL: %s\n",SDL_GetError());
		return(255);
	}
	atexit(CleanUp);
	...
	if (Mix_OpenAudio(audio_rate, audio_format, audio_channels, audio_buffers) < 0) {
		t_print(T_DEBUG, "Couldn't open audio: %s\n", SDL_GetError());
		return(2);
	} else {
	...
	while (argv[i]) {
		...
		if ( music == NULL ) {
			t_print(T_DEBUG, "Couldn't load %s: %s\n",
				argv[i], SDL_GetError());
			return(2);
		}
